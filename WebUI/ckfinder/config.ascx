...
<script runat="server">
    ...
    public override void SetConfig()
    {
        ...
        LicenseName = "Duong2492DD";
        LicenseKey = "LKR62CYBV7N7KHLJDVR6FEXT689Y719V";
        string time = DateTime.Now.Year.ToString() + DateTime.Now.Month.ToString() + DateTime.Now.Day.ToString();
        //string user = Common.GetUserNameFromSession();
        string user = Context.User.Identity.Name;
        // The base URL used to reach files in CKFinder through the browser.
        if (!string.IsNullOrEmpty(user))
        {
            BaseUrl = "~/ckfinder/userfiles/admin/" + user + "/" ;
            Thumbnails.Url = "~/ckfinder/userfiles/" + "_thumbs/admin/" + user + "/";
            //BaseUrl = "~/ckfinder/userfiles/admin/" + user + "/" + time + "/";
            //Thumbnails.Url = "~/ckfinder/userfiles/" + "_thumbs/admin/" + user + "/" + time + "/";
        }
        else
        {
            BaseUrl = "~/ckfinder/userfiles/user/";
            Thumbnails.Url = "~/ckfinder/userfiles/" + "_thumbs/user/";
        }
